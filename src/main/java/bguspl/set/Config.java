...
public class Config {
  ...
  private static Properties loadProperties(String filename, Logger logger) {
    ...
    if (filename == null || filename.isEmpty()) logger.severe(
      "running with default configuration."
    ); else try (InputStream is = Files.newInputStream(Paths.get(filename))) {
      properties.load(is);
    } catch (IOException e) {
      logger.severe(
        "cannot read configuration file " + filename + " trying from resources."
      );
      try (
        ...
        properties.load(is);
        logger.severe(
          "configuration file was loaded from resources directory."
        );
  ...
  public Config(Logger logger, Properties properties) {
    ...
    String logFormat = properties.getProperty(
      "LogFormat",
      "[%1$tT.%1$tL] [%2$-7s] %3$s%n"
    );
    Main.setLoggerLevelAndFormat(logger, logLevel, logFormat);

    // for debugging
    randomSpinMin =
      Long.parseLong(properties.getProperty("RandomSpinMin", "0"));
    randomSpinMax =
      Long.parseLong(properties.getProperty("RandomSpinMax", "0"));
    if (randomSpinMax < randomSpinMin || randomSpinMin < 0) logger.severe(
      "invalid random spin cycles: max: " +
      randomSpinMax +
      " min: " +
      randomSpinMin
    );
    ...
    featureCount =
      Integer.parseInt(properties.getProperty("FeatureCount", "4"));
    deckSize = (int) Math.pow(featureSize, featureCount);

    // gameplay settings
    humanPlayers =
      Integer.parseInt(properties.getProperty("HumanPlayers", "2"));
    computerPlayers =
      Integer.parseInt(properties.getProperty("ComputerPlayers", "0"));
    players = humanPlayers + computerPlayers;

    hints = Boolean.parseBoolean(properties.getProperty("Hints", "False"));
    turnTimeoutMillis =
      (long) (
        Double.parseDouble(properties.getProperty("TurnTimeoutSeconds", "60")) *
        1000.0
      );
    turnTimeoutWarningMillis =
      (long) (
        Double.parseDouble(
          properties.getProperty("TurnTimeoutWarningSeconds", "60")
        ) *
        1000.0
      );
    pointFreezeMillis =
      (long) (
        Double.parseDouble(properties.getProperty("PointFreezeSeconds", "1")) *
        1000.0
      );
    penaltyFreezeMillis =
      (long) (
        Double.parseDouble(
          properties.getProperty("PenaltyFreezeSeconds", "3")
        ) *
        1000.0
      );
    tableDelayMillis =
      (long) (
        Double.parseDouble(properties.getProperty("TableDelaySeconds", "0.1")) *
        1000.0
      );
    endGamePauseMillies =
      (long) (
        Double.parseDouble(properties.getProperty("EndGamePauseSeconds", "5")) *
        1000.0
      );

    // ui settings
    String[] names = properties
      .getProperty("PlayerNames", "Player 1, Player 2")
      .split(",");
    playerNames = new String[players];
    Arrays.setAll(
      playerNames,
      i -> i < names.length ? names[i].trim() : "Player " + (i + 1)
    );
    ...
    playerCellWidth =
      Integer.parseInt(properties.getProperty("PlayerCellWidth", "300"));
    playerCellHeight =
      Integer.parseInt(properties.getProperty("PlayerCellHeight", "40"));
    ...
    for (int i = 0; i < players; i++) {
      ...
      String playerKeysString = properties.getProperty(
        "PlayerKeys" + (i + 1),
        defaultCodes
      );
      if (playerKeysString.length() > 0) {
        String[] codes = playerKeysString.split(",");
        if (codes.length != tableSize) logger.severe(
          "warning: player " +
          (i + 1) +
          " keys (" +
          codes.length +
        ...
        for (
          int j = 0;
          j < Math.min(codes.length, tableSize);
          ++j
        ) playerKeys[i][j] = Integer.parseInt(codes[j]); // parse the key codes string
      }
    }
  }
