...
import { Class, isClass, makeClassEnv, Object, isObject, makeObjectEnv, isSymbolSExp, isClosure, makeClosureEnv, Closure, Value } from "./L3-value";
import { applyPrimitive } from "./evalPrimitive";
import { allT, first, rest, isEmpty, isNonEmptyList } from "../shared/list";
import { isFailure, Result, makeOk, makeFailure, bind, mapResult } from "../shared/result";
...
const evalClass = (exp: ClassExp, env: Env): Result<Value> =>
    makeOk(makeClassEnv(exp.fields, exp.methods, env)); // added
...
const applyProcedure = (proc: Value, args: Value[]): Result<Value> =>
    ...
    isObject(proc) ? applyObject(proc,args): //added
    isClass(proc) ? applyClass(proc, args): //added
    makeFailure(`Bad procedure ${format(proc)}`);

const applyObject = (proc: Object, args: Value[]): Result<Value> => { //added
    if (!isNonEmptyList<Value>(args))
        return makeFailure("there is no method name")
    if (!isSymbolSExp(args[0]))
        return makeFailure("The 1st argument is not the method name")
    const selectedName = args[0].val;
    const methodNames = map((b: Binding) => b.var.var , proc.methods);
    if(!methodNames.includes(selectedName))
        return makeFailure(`Unrecognized: ${selectedName}`)
...
const applyClass = (proc: Class, args: Value[]): Result<Value> => //added
    !isNonEmptyList(args) ? makeFailure('cannot create object with no fields') :
