...
import { ClassExp, isClassExp, makeBinding, isCExp, isLetExp } from "./L3-ast";
...
import { Class, isClass, makeClass, Object, isObject, makeObject, isSymbolSExp, isClosure, makeClosure, Closure, Value } from "./L3-value";
import { allT, first, rest, isEmpty, List, isNonEmptyList } from '../shared/list';
...
const evalClass = (exp: ClassExp, env: Env): Result<Value> =>
    makeOk(makeClass(exp.fields, exp.methods)); //added
...
const L3applyProcedure = (proc: Value, args: Value[], env: Env): Result<Value> =>
    ...
    isObject(proc) ? applyObject(proc, args, env) : //added
    isClass(proc) ? applyClass(proc, args, env) : //added
...
const applyClass = (proc: Class, args: Value[], env: Env): Result<Value> => { // added
    ...
    const methodsRenamedSub = allT(isProcExp, methods) ? map((method) => makeProcExp(method.args, substitute(renameExps(method.body), fields, litArgs)), methods): methods;
    return makeOk(makeObject(zipWith(makeBinding, methodsNames, methodsRenamedSub)));
};

const applyObject = (proc: Object, args: Value[], env: Env): Result<Value> => { //added
    if (!isNonEmptyList<Value>(args))
        return makeFailure("there is no method name");
    const methodNames = map((b: Binding) => b.var.var, proc.methods);
    const selectedName = first(args);
    if(!isSymbolSExp(selectedName))
        return makeFailure(`couldnt run method`);
