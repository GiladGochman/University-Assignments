...
export type CompoundExp = AppExp | IfExp | ProcExp | LetExp | LitExp | ClassExp; //added classexp
...
export type ClassExp = {tag: "ClassExp"; fields : VarDecl[]; methods : Binding[]; } //added
...
export const makeClassExp = (fields : VarDecl[], methods : Binding[]): ClassExp =>
    ({ tag: "ClassExp", fields : fields, methods : methods }); //added
...
export const isClassExp = (x: any): x is ClassExp => x.tag === "ClassExp"; // added
...
export const isCompoundExp = (x: any): x is CompoundExp =>
    isAppExp(x) || isIfExp(x) || isProcExp(x) || isLitExp(x) || isLetExp(x) || isClassExp(x); // added
...
export const parseClassExp = (varDecls: Sexp, bindings: Sexp): Result<ClassExp> => {
...
const unparseClassExp = (ce: ClassExp) : string => 
    `(class (${ce.fields.map((c: VarDecl) => c.var).join(" ")}) (${ce.methods.map((b: Binding) => `(${b.var.var} ${unparseL3(b.val)})`).join(" ")}))`; //added
