...
import { ClassExp, isClassExp, Binding, ProcExp, isPrimOp, CExp, PrimOp, VarDecl } from './L3-ast';
...
export type Class = {tag: "Class"; fields: VarDecl[]; methods: Binding[]; env: Env;}; //added
export const makeClass = (fields: VarDecl[], methods: Binding[]): Class => ({//added
    tag: "Class", fields: fields, methods: methods, env: makeEmptyEnv(),});
export const makeClassEnv = (fields: VarDecl[], methods: Binding[], env: Env): Class => ({ //added
    tag: "Class", fields: fields, methods: methods, env: env,});
export const isClass = (x: any): x is Class => x.tag === "Class";// added
export type Object = {tag: "Object", methods: Binding[], env: Env}; //added
export const makeObject = (methods: Binding[]): Object => ({ //added
    tag: "Object", methods: methods, env: makeEmptyEnv()});
export const makeObjectEnv = (methods: Binding[], env: Env): Object => ({//added
    tag: "Object", methods: methods, env: env});
  export const isObject = (x: any): x is Object => x.tag === "Object"; // added
...
export type SExpValue = number | boolean | string | PrimOp | Closure | SymbolSExp | EmptySExp | CompoundSExp | Class | Object; //added
export const isSExp = (x: any): x is SExpValue =>
    typeof(x) === 'string' || typeof(x) === 'boolean' || typeof(x) === 'number' ||
    isSymbolSExp(x) || isCompoundSExp(x) || isEmptySExp(x) || isPrimOp(x) || isClosure(x) || isClass(x) || isObject(x); //added
...
export const valueToString = (val: Value): string =>
    ...
    isClass(val) ? `Class` : //added
    isObject(val) ? `Object` : // added
    val;
