...
export const parseClassExp = (fields: Sexp, methods: Sexp): Result<ClassExp> => {
    ...
    return isArray(fields) && isNonEmptyList(fields) && allT(isString, fields) ? mapv(bindingsResult, ((bindings: Binding[]) => 
            makeClassExp(map(makeVarDecl, fields), bindings))) :
        makeFailure(`Invalid fileds for ClassExp ${format(fields)}`); ;
}

export const isGoodBindings = (bindings: Sexp): bindings is [string, Sexp][] =>
    isArray(bindings) &&
...
export const unparseClassExp = (ce: ClassExp): string => 
    `(class (${map((f: VarDecl) => f.var, ce.fields).join(" ")}) (${map((m: Binding) =>
         `(${m.var.var} ${unparseL3(m.val)})`, ce.methods).join(" ")}))`;
